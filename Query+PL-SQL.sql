...
/
-- Name of students who have advisor having minimum salary 
set serveroutput on
declare
cursor c1 is select * from instructor where salary = (select min(salary) from instructor);
cursor c2 is select * from advisor;
cnt number := 0;
adv_name instructor.name%type;
stud_name student.name%type;
min_sal instructor.salary%type;
begin
select min(salary) into min_sal from instructor;
...
for i in c1 loop
    ...
    for j in c2 loop
        if i.id like j.i_id
            then
            select name into stud_name from student where id = j.s_id;
            dbms_output.put_line('Advisor: ' || adv_name || ', Student: ' || stud_name);
        end if;
    end loop;
end loop;
end;
...
--Trigger
-- Update the cgpa attribute of student on insert/update of takes relation
create or replace trigger updCgpa 
before insert or update on takes 
referencing old as o new as n 
for each row
declare
tot_grad numeric(5, 2);
cnt number;
grad takes.grade%type;
begin
    select sum(case grade
        ...
        end), count(*) into tot_grad, cnt from takes where id = :n.id;
    tot_grad := tot_grad + (case :n.grade
            when 'A' then 4
            when 'B' then 3.5
            when 'C' then 3
            when 'D' then 2.5
    ...
    cnt := cnt + 1;
    update student set cgpa = tot_grad / cnt where id = :n.id;
end;
/
